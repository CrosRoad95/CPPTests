...
public:
    ...
    // Functions
    void           Split(const SString& strDelim, std::vector<SString>& outResult, uint uiMaxAmount = 0, uint uiMinAmount = 0) const;
...
public:
    TSplitString() {}
    TSplitString(const STRING_TYPE& strInput, const STRING_TYPE& strDelim, uint uiMaxAmount = 0, uint uiMinAmount = 0)
    {
        Split(strInput, strDelim, uiMaxAmount, uiMinAmount);
    }

    void Split(const STRING_TYPE& strInput, const STRING_TYPE& strDelim, uint uiMaxAmount = 0, uint uiMinAmount = 0)
    {
